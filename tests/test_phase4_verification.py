...
def test_phase4_complete_workflow():
    ...
    try:
        ...
        orchestrator.code_generator.emit_event(EventType.KNOWLEDGE_SUGGESTION, {
        ...
        status_result = orchestrator.process_request('knowledge_manager', {
            'action': 'get_queue_status',
            'project_id': project_id
        })
        assert status_result['status'] == 'success'
        assert status_result['pending'] > 0, "Should have pending suggestions"
        print(f"   Queue: {status_result['pending']} pending, {status_result['approved']} approved, {status_result['rejected']} rejected")
        ...
        orchestrator.socratic_counselor.emit_event(EventType.KNOWLEDGE_SUGGESTION, {
        ...
        final_status = orchestrator.process_request('knowledge_manager', {
            'action': 'get_queue_status',
            'project_id': project_id
        })
        print(f"   Final: {final_status['pending']} pending, {final_status['approved']} approved, {final_status['rejected']} rejected")
...
def test_agent_has_suggest_method():
    ...
    try:
        ...
        for agent in agents_to_check:
            assert hasattr(agent, 'suggest_knowledge_addition'), \
