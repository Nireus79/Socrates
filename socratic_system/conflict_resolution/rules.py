...
CONFLICT_RULES = {
    'databases': [
        ...
        'cassandra', 'dynamodb', 'elasticsearch', 'neo4j', 'firestore'
    ],
    'frontend_frameworks': [
        'react', 'vue', 'angular', 'svelte', 'ember', 'backbone',
        'next.js', 'nuxt', 'gatsby', 'astro', 'quasar'
    ],
    'backend_frameworks': [
        'django', 'flask', 'fastapi', 'express', 'spring', 'rails',
        'laravel', 'asp.net', 'go', 'rust-actix', 'java-spring'
    ],
    'languages': [
    ...
    'package_managers': [
        'npm', 'yarn', 'pip', 'poetry', 'cargo', 'maven', 'gradle'
    ],
    'testing_frameworks': [
        'pytest', 'jest', 'jasmine', 'mocha', 'unittest', 'testng'
    ],
    'build_tools': [
        'webpack', 'vite', 'rollup', 'parcel', 'maven', 'gradle'
    ]
}
...
def find_conflict_category(item1: str, item2: str) -> Optional[str]:
    ...
    for category, items in CONFLICT_RULES.items():
        ...
        item1_in_category = any(
            item1_str in tech.lower() or tech.lower() in item1_str
            for tech in items
        )
        item2_in_category = any(
            item2_str in tech.lower() or tech.lower() in item2_str
            for tech in items
        )
